...
export function* metrics(status: FDBStatus): IterableIterator<Metric> {
  ...
  yield {
    type: 'counter',
    name: 'fdb_generation',
    help: 'cluster generation',
    values: [{value: status.cluster?.generation}],
  };
  ...
  yield* layerMetrics(status.cluster?.layers);
}
...
function* layerMetrics(
  layer: NonNullable<FDBStatus['cluster']>['layers'],
): IterableIterator<Metric> {
  if (!layer) {
    return;
  }
  ...
  yield {
    type: 'gauge',
    name: 'fdb_layer_valid',
    help: 'Layer valid',
    values: [{value: layer._valid ? 1 : 0}],
  };

  yield {
    type: 'gauge',
    name: 'fdb_backup_total_workers',
    help: 'Backup total workers',
    values: [{value: layer.backup?.total_workers}],
  };

  yield {
    type: 'gauge',
    name: 'fdb_backup_instances_running',
    help: 'Backup intances',
    values: [{value: layer.backup?.instances_running}],
  };

  yield {
    type: 'gauge',
    name: 'fdb_backup_last_updated',
    help: 'Backup last updated',
    values: [{value: layer.backup?.last_updated}],
  };

  yield {
    type: 'gauge',
    name: 'fdb_backup_paused',
    help: 'Backup paused',
    values: [{value: layer.backup?.paused ? 1 : 0}],
  };

  //yield* backupTagsMetrics(layer);

}

/*
function* backupTagsMetrics(
  layer: NonNullable<FDBStatus['cluster']>['layers'],
): IterableIterator<Metric> {
  if (!layer) {
    return;
  ...
  var tags = layer.backup?.tags;

  yield {
    type: 'gauge',
    name: 'fdb_backup_running_backup',
    help: 'FDB running backup',
    values: Object.entries(layer.backup?.tags).map(([tag, status]) => ({
      labels: {tag, current_status: status.current_status, current_container: status.current_status},
      value: status.running_backup ? 1 : 0,
    })),
  };

  yield {
    type: 'counter',
    name: 'fdb_backup_range_bytes_written',
    help: 'Backup range bytes written',
    values: Object.entries(tags).map(([tag, status]) => ({
      labels: {tag, current_status: status.current_status, current_container: status.current_status},
      value: status.range_bytes_written,
    })),
  };

  yield {
    type: 'counter',
    name: 'fdb_backup_last_restorable_seconds_behind',
    help: 'Backup last restirable seconds behind',
    values: Object.entries(tags).map(([tag, status]) => ({
      labels: {tag, current_status: status.current_status, current_container: status.current_status},
      value: status.last_restorable_seconds_behind,
    })),
  };

  yield {
    type: 'counter',
    name: 'fdb_backup_mutation_log_bytes_written',
    help: 'Backup mutation log bytes written',
    values: Object.entries(tags).map(([tag, status]) => ({
      labels: {tag, current_status: status.current_status, current_container: status.current_status},
      value: status.mutation_log_bytes_written,
    })),
  };
}
*/
...
function* processesRolesMetrics(
  ...
  yield* grvProxyProcessMetrics(processes);
  yield* commitProxyProcessMetrics(processes);
}

function* commitProxyProcessMetrics (
  processes: NonNullable<FDBStatus['cluster']>['processes'],
): IterableIterator<Metric> {
  if (!processes) {
    return;
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_commitproxy_latency_count',
    help: 'Commit Proxy latency count',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('commit_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.commit_latency_statistics?.count,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_commitproxy_latency_avg',
    help: 'Commit Proxy latency avg',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('commit_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_commitproxy_latency_max',
    help: 'Commit Proxy latency max',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('commit_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
...
function* grvProxyProcessMetrics (
  processes: NonNullable<FDBStatus['cluster']>['processes'],
): IterableIterator<Metric> {
  if (!processes) {
    return;
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_grvproxy_latency_batch_count',
    help: 'Grv Proxy batch latency count',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.grv_latency_statistics?.batch?.count,
      })),
    ), 
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_grvproxy_latency_batch_avg',
    help: 'Grv Proxy batch latency avg',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_grvproxy_latency_batch_max',
    help: 'Grv Proxy batch latency max',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.grv_latency_statistics?.batch?.max,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_grvproxy_latency_default_count',
    help: 'Grv Proxy default latency count',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.grv_latency_statistics?.default?.count,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_grvproxy_latency_default_avg',
    help: 'Grv Proxy default latency avg',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_grvproxy_latency_default_max',
    help: 'Grv Proxy default latency max',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('grv_proxy')).map((status) => ({
        labels: {processId, address: processStatus.address},
...
function* storageProcessesMetrics(
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_storage_bytes_queried_total',
    help: 'Storage process bytes queried total',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.bytes_queried?.counter,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_bytes_queried_hz',
    help: 'Storage process bytes queried per second',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.bytes_queried?.hz,
      })),
    ),
  };
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_storage_finished_queries_total',
    help: 'Storage process finished queries total',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.finished_queries?.counter,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_finished_queries_hz',
    help: 'Storage process finished queries per second',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.finished_queries?.hz,
      })),
    ),
  };
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_storage_keys_queried_total',
    help: 'Storage process keys_queried total',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.keys_queried?.counter,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_keys_queried_hz',
    help: 'Storage process keys queried per second',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.keys_queried?.hz,
      })),
    ),
  };
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_storage_mutation_bytes_total',
    help: 'Storage process mutation bytes total',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.mutation_bytes?.counter,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_mutation_bytes_hz',
    help: 'Storage process mutation bytes per second',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.mutation_bytes?.hz,
      })),
    ),
  };
  ...
  yield {
    type: 'counter',
    name: 'fdb_process_storage_mutations_total',
    help: 'Storage process mutations total',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.mutations?.counter,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_mutations_hz',
    help: 'Storage process mutations per second',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.mutations?.hz,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_query_queue_max',
    help: 'Storage process query queue max',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
        value: status.query_queue_max,
      })),
    ),
  };
  ...
  yield {
    type: 'gauge',
    name: 'fdb_process_storage_read_latency_avg',
    help: 'Storage process read latency avg',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
  ...
  yield {
    ...
    help: 'Storage process read latency max',
    values: Object.entries(processes).flatMap(([processId, processStatus]) =>
      (processStatus.roles ?? []).filter(isRole('storage')).map((status) => ({
        labels: {processId, address: processStatus.address},
